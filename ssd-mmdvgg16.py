...
from utils.utils import convert_secs2time, time_string, time_file_str
...
parser.add_argument('--data', type=str, default='E:\imagenet', help='Path to dataset')
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='mmdvgg',
                    choices=model_names,
...
parser.add_argument('--schedule', type=int, nargs='+', default=[45, 68],
...
def main():
    ...
    #model = import_module(f'models.{args.arch}').resnet(args.cfg)
    ...
    if use_cuda:
        if not args.distributed:
            if args.arch.startswith('alexnet') or args.arch.startswith('mmdvgg'):
